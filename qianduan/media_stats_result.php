<?php
 //include_once('check_user.php');
  ...
  if(!isset($_POST['kids'])||!isset($_POST['a_type'])||!isset($_POST['start'])||!isset($_POST['end'])){
      echo "<script>alert('查询条件不足！');location.href='media_stats.php';</script>";
      return;
  }
...
<div class="Content">
      <div class="output stats_navi">
	       <ul>
			   ...
			   <li class="active"><a href="media_stats.php">媒体统计</a></li>
			   <li><a href="weibo_stats.php">微博统计</a></li>
			   <li><a href="info_graph.php">统计图</a></li>
		   </ul>
	  </div>
      <form action="media_stats_result.php" method="post" name="queryForm">
              <div class="output">
				    <label>关&nbsp;键&nbsp;字：</label>
		...
		<?php
		  if($user_type!=2){
		?>
		<form action="write_media_stats.php" method="post">
				<div class="output">
				      ...
				      <input type="text" name="file_name" id="file_name" size="60" value="<?php echo $date."_媒体统计报告"; ?>" />
...
<div class="Content" style="padding-top:0px;">
		<table border="0" cellspacing="0" cellpadding="0" width="900">
			<thead class="Header Center">
			 ...
			 <tbody>
			 <?php
				  $query="select sum(total_num) as total_num,sum(positive_num) as positive_num,sum(negative_num) as negative_num,sum(neutral_num) as neutral_num,uk_id,article_class,media from media_stats where uk_id in (".$uk_ids.") and stats_time>$start_time and stats_time<$end_time ";
				  if($type!=0){
				        $query.="and article_class=$type ";
				  }
				  $query.="group by uk_id,article_class,media order by uk_id asc,article_class asc";
				  $res=mysql_query($query);
