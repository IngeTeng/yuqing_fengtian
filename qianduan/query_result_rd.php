				...
				<div class="Content" style="padding-top:0px;">
				  <table border="0" cellspacing="0" cellpadding="0" width="900">
				     <thead class="Header Center">
				         <tr>
						      <td width="4%">选择</td><td width="40%">文章标题</td><td width="10%">媒体名称</td><td width="10%">文章调性</td><td width="10%">关键字</td><td width="10%">文章类型</td>
						      <td width="12%">
						      <?php
						      if ($order == 1)
						      {
					 ...
					 <tbody>
					    <?php					    
							 ...
							 $query2="select article_id,article_property,id,uk_id,audit_status,a_type from $key_table where uk_id in (".$uk_ids.") and $time_item>$start_time and $time_item<$end_time ";
							 if($audit!="all"){
							...
							// echo($query2);
							 ...
							 while($row2=mysql_fetch_array($res2)){
								 ...
								 if($author_type=="all"){
									  ...
									  if($type==4){
									       $query3="select article_title,article_url,article_pubtime,media,author,article_addtime from $article_table where article_id=$article_id";
									  }
									  else{
									       $query3="select article_title,article_url,article_pubtime,media,article_summary,article_addtime from $article_table where article_id=$article_id";
									  }
								 }else{							     
									  $query3="select article_title,article_url,article_pubtime,media,rz_info,article_addtime from $article_table where article_id=$article_id and isV=$author_type";
								 }
