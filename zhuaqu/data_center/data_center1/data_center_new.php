...
ini_set('error_log', dirname(__FILE__) . '/error_log_new.txt'); //将出错信息输出到一个文本文件

while(true) {
    $log_handler = LOG_HANDLER."_new_". date('Y_m_d', time());
    ...
    $files = glob(TARGET_FAIL_PATH. '*.json');
...
//            print_r($request);
            ...
            if(!empty($response)) {
                ...
                if($response_array['status'] == REQUEST_OK) {
                    ...
                    echo "$response", PHP_EOL;
                    // 请求失败进行转移
//                    if (!is_dir(BASE_PATH. 'fail_new/')) {
//                        mkdir(BASE_PATH. 'fail_new/');
//                    }
//                    $file_name = str_replace(TARGET_FAIL_PATH, '', $file);
//                    // 使用rename进行文件转移,速度快
//                    rename($file, BASE_PATH. 'fail_new/'. $file_name);
//                    // unlink($file);
                }
            }
            else {
