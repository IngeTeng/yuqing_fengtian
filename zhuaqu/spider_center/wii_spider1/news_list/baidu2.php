 ...
 * @createtime 2016-11-13  18:05:53
 * @updatetime 2016-11-13  18:05:53
 * @version 1.0
 * @Description
 * 同baidu1,只是url不同
 * 
...
$log = Log::Init(Configure::getLogHandle(BASE_PATH, 'baidu2'), LOG_LEVEL);
...
$url_host = Configure::$url_hosts['baidu2'];        // 获取爬取网址
...
foreach($task_array['keywords'] as $keyword) {
    ...
    while($page <= Configure::PAGE_NUM) {
        $spider_url = sprintf($url_host, urlencode($key_word), ($page-1)*Configure::ONE_PAGE_NUM, Configure::ONE_PAGE_NUM);        // 构造url
        $page++;
