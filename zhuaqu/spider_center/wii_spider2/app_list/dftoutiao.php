...
/**
 * @filename dftoutiao.php 
 * @encoding UTF-8 
 * @author WiiPu yjl 
 * @createtime 2018-01-29 16:00
 * @updatetime 
 * @version 1.0
 * @Description
 * 东方头条
...
$clist = array('qiche', 'toutiao', 'redian', 'shehui', 'guonei', 'shishang', 'caijing', 'keji');
$count = count($clist);
for($i = 0; $i < $count; $i++) {//新闻类型
    for ($j=1; $j <= 2 ; $j++) { //页数
        $spider_url = sprintf($url_host, $clist[$i], $j);       // 构造url
        $spider_result = get_html($spider_url,'','','','', true);     // 获取html
        //file_put_contents('ssssss', $spider_result);
...
//热文
$spider_url2 = "https://tjv1.dftoutiao.com/newsranklist/getgrn.d?colid=1236&pgsize=100"; 
//$spider_result = get_html($spider_url2,'','','','', true);     // 获取html
$spider_result = file_get_contents("compress.zlib://".$spider_url2);     // 获取html
$spider_result_path = Configure::get_spider_result_path(BASE_PATH);     // 获取保存路径

$content_request['timestamp'] = time();
$content_request['collect_url'] = $spider_url2; 
